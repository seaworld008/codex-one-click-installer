...
[![OpenAI Codex CLI](https://img.shields.io/badge/OpenAI-Codex%20CLI%20%2B%20App-412991)](https://github.com/seaworld008/codex-one-click-installer)
...
Windows / macOS one-click installer for OpenAI Codex CLI, with optional Codex Windows App installation.

这个仓库提供 Windows 和 macOS 的 Codex CLI 一键安装脚本；在 Windows 上，如果你提供 Codex App 安装器或下载地址，脚本也会在能力范围内尽量安装 Codex Windows App。它适合希望快速安装 Codex CLI、又不想手动处理 Git / Node.js / Python / npm registry / 网络镜像的用户。脚本会优先使用官方可获取的安装包；当系统或架构不满足官方安装条件时，会在下载和安装前尽早提示用户。
...
- CLI + App：默认安装 Codex CLI；Windows 可通过本地安装器或 `CODEX_APP_INSTALLER_URL` 额外安装 Codex App。
...
| --- | --- | --- |
| Codex CLI | 支持 | 支持 |
| Codex App | 可选：提供 `Codex Installer.exe` 或 `CODEX_APP_INSTALLER_URL` 后尽量安装 | 暂未自动安装 |
| Codex Skills | 可选：提供 `codex-skills.zip` 或 `CODEX_SKILLS_URL` 后安装 | 可选：提供 `codex-skills.zip` 或 `CODEX_SKILLS_URL` 后安装 |
