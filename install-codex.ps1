...
# ========== 公开官方版本配置 ==========
$ModernGitVersion = "2.55.0"
$ModernGitTag = "v2.55.0.windows.1"
$ModernNodeVersion = "24.18.0"
$ModernPythonVersion = "3.14.6"
...
$WorkDir = Join-Path $env:TEMP "codex-installer"
...
function New-DownloadPlan {
    ...
    if ($Mode.IsLegacy) {
        ...
        $plan = [ordered]@{
            GitUrl = "https://github.com/git-for-windows/git/releases/download/$LegacyGitTag/$gitFile"
            NodeUrl = "https://nodejs.org/dist/v$LegacyNodeVersion/$nodeFile"
        ...
        $nodeFile = "node-v$ModernNodeVersion-$Arch.msi"
        $pythonArch = $(if ($Arch -eq "arm64") { "arm64" } else { "amd64" })
        $pythonFile = "python-$ModernPythonVersion-$pythonArch.exe"

        $plan = [ordered]@{
            GitUrl = "https://github.com/git-for-windows/git/releases/download/$ModernGitTag/$gitFile"
            NodeUrl = "https://nodejs.org/dist/v$ModernNodeVersion/$nodeFile"
            PythonUrl = "https://www.python.org/ftp/python/$ModernPythonVersion/$pythonFile"
            GitFile = $gitFile
            NodeFile = $nodeFile
            PythonFile = $pythonFile
            RequiredNodeMajor = 16
            RequiredPython = $ModernPythonVersion
        }
    }

    $plan["GitUrl"] = Get-ConfigValue -Name "GitUrl" -EnvName "CODEX_GIT_URL" -DefaultValue $plan["GitUrl"]
    $plan["NodeUrl"] = Get-ConfigValue -Name "NodeUrl" -EnvName "CODEX_NODE_URL" -DefaultValue $plan["NodeUrl"]
    $plan["PythonUrl"] = Get-ConfigValue -Name "PythonUrl" -EnvName "CODEX_PYTHON_URL" -DefaultValue $plan["PythonUrl"]
    ...
    $plan["NpmRegistry"] = Get-ConfigValue -Name "NpmRegistry" -EnvName "CODEX_NPM_REGISTRY" -DefaultValue ""
...
function Download-File {
    ...
    try {
        ...
        Write-Warning "Invoke-WebRequest 下载失败，改用 WebClient 重试：$($_.Exception.Message)"
        $client = New-Object System.Net.WebClient
        $client.DownloadFile($Url, $OutFile)
    }

    if (-not (Test-Path $OutFile) -or ((Get-Item $OutFile).Length -lt 1024KB)) {
        throw "下载失败或文件异常：$Name -> $OutFile"
    }
...
function Install-CodexCli {
    ...
    if (-not [string]::IsNullOrWhiteSpace($Plan.NpmRegistry)) {
        & $npm config set registry $Plan.NpmRegistry
    }

    & $npm install -g "@openai/codex@latest"
    if ($LASTEXITCODE -ne 0) { throw "npm install -g @openai/codex@latest 执行失败。" }
}
...
try {
    ...
    if (-not $SkipGit) {
        if (Get-CommandPath "git") {
            ...
            Download-File -Name "Git for Windows" -Url $plan.GitUrl -OutFile $gitFile
    ...
    if (Test-NodeReady) {
        ...
        Download-File -Name "Node.js" -Url $plan.NodeUrl -OutFile $nodeFile
    ...
    if (-not $SkipPython) {
        if (Test-PythonReady) {
            ...
            Download-File -Name "Python" -Url $plan.PythonUrl -OutFile $pythonFile
